...
class OrderAPITests(TestCase):
    ...
    def display_time_resources_queue(self):
        ...
        table = PrettyTable()
        table.field_names = ["Resource Item Code", "Segment", "Order", "Start Time", "End Time", "Resource Model"]
        for trq in trqs:
            table.add_row([trq.resource_item_code, trq.segment, trq.order_number, trq.segment_start, trq.segment_end, trq.resource_model])
...
############################################################################################################
    ...
    def test_create_order_and_order_items(self):
        ...
        first_response = self.submit_order(first_order_data)
        # Validate the results after first order submission
        self.assertEqual(TimeResourcesQueue.objects.filter(resource_item_code=self.timeResourceItem_cher1).count(), 3, "3 trq for chair after 1st order submit")
        ...
        second_response = self.submit_order(second_order_data)
        # Validate the results after second order submission
        self.assertEqual(TimeResourcesQueue.objects.filter(resource_item_code=self.timeResourceItem_cher1).count(), 5, "5 trq for chair after 2nd order submit")
